package com.excilys.formation.computerdb.constants;

public class Paths {
	public static final String PATH_DASHBOARD 		= "/computer-database/access";
	public static final String PATH_COMPUTER_ADD 	= "/computer-database/access/add";
	public static final String PATH_COMPUTER_EDIT 	= "/computer-database/access/edit";
	public static final String PATH_COMPUTER_DELETE = "/computer-database/access/delete";
	public static final String PATH_COMPUTER_SEARCH = "/computer-database/access/search";
	public static final String PATH_COMPUTER_LOGIN 	= "/computer-database/access/login";
	public static final String PATH_COMPUTER_LOGOUT = "/computer-database/access/logout";

	public static final String PREVIOUS_PAGE 		= "prev";
	public static final String NEXT_PAGE 			= "next";

	public static final String PATH_DASHBOARD_RESET = "/computer-database/access?reset=true";
}
