...
public class ComputerMapper implements RowMapper<Computer> {
	...
	public Computer map(ResultSet rs, Company company) throws SQLException {
		...
		return new Computer.Builder().id(id).name(name).intro(intro).outro(outro).company(company).build();
	}
	...
	/**
	 * Maps a ResultSet into a Computer. WARNING: the company will not be set and so has to be set
	 * manually after.
	 */
	@Override
	public Computer mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		Company c = new Company.Builder().id(cid).build();

		return new Computer.Builder().id(id).name(name).intro(intro).outro(outro).company(c).build();
	}
}
